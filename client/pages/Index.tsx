      ...
      <div className="container mx-auto px-4 py-3 sm:py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 rounded-xl border bg-white/90 px-3 py-1 shadow">
            ...
            <div className="text-xs sm:text-sm font-black text-primary tabular-nums">
              {timeStr}
            </div>
          </div>
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            maskImage:
              "radial-gradient(closest-side, transparent 44%, black 46%)",
            WebkitMaskImage:
              "radial-gradient(closest-side, transparent 44%, black 46%)",
            filter: "blur(0.5px)",
          }}
...
function SectionTitle({
  ...
  return (
    <div className="text-center py-6 sm:py-8">
      ...
      <h2 className="text-2xl font-bold tracking-tight text-white sm:text-3xl md:text-4xl 2xl:text-5xl">
        {children}
      </h2>
      {subtitle ? (
        <p className="mx-auto mt-2 max-w-2xl text-sm text-muted-foreground sm:text-black md:text-lg">
          {subtitle}
        </p>
      ) : null}
    </div>
  ...
  if (variant === "emergency") {
    return (
      <div className="rounded-2xl p-[2px] bg-gradient-to-br from-red-500/40 to-red-400/20 border border-red-400/40">
        <div
          className={`rounded-2xl border border-red-400/30 bg-white shadow-sm ${className}`}
        >
          {children}
        </div>
      </div>
...
function WeatherSlide() {
  ...
  return (
    <div className="min-h-[calc(100vh-88px)] flex flex-col items-center justify-center py-8">
      ...
      <SectionTitle
        children="Temperature Alert"
        subtitle="درجہ حرارت کی انتباہ"
      />
      <p className="animate-pulse text-center text-lg font-semibold text-red-600">
        HIGH TEMPERATURES EXPECTED
      </p>
      <p className="mb-6 text-center text-base text-red-600/80">
        .ہت زیادہ درجہ حرارت کی توقع ہے
      </p>
      <div className="grid w-full max-w-6xl grid-cols-1 gap-4 sm:grid-cols-3">
        {temps.map((t) => (
          <Card key={t.location} variant="accent" className="p-6 text-center">
            ...
            {loading ? (
              <div className="my-2 text-2xl text-muted-foreground">
                Loading...
              </div>
            ) : (
              <>
                <div className="my-2 text-5xl font-black text-red-600 sm:text-6xl 2xl:text-7xl">
                  {Math.round(t.temp)}°C
                </div>
...
function HealthSlide() {
  ...
  return (
    <div className="min-h-[calc(100vh-88px)] flex flex-col justify-center py-8">
      <SectionTitle
        children="Essential Health Tips"
        subtitle="اہم صحت کے نکات"
      />
        ...
        {[tipsA, tipsB].map((list, col) => (
          <div key={col} className="flex flex-col gap-4">
            {list.map(([icon, title, ar], i) => (
              <Card
                key={i}
                className="border-l-4 border-primary p-5 transition-all will-change-transform"
                data-animate-item
              >
      ...
      try {
        ...
        if (timings) {
          const parseTime = (timeStr: string) => {
            ...
            return {
              time: new Date(0, 0, 0, h, m).toLocaleTimeString("en-US", {
                hour: "2-digit",
                minute: "2-digit",
              }),
              minutes: h * 60 + m,
    ...
    <div className="min-h-[calc(100vh-88px)] flex flex-col justify-center py-8">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-6">
        {data.map((p, i) => (
          ...
          >
            <div className="text-sm font-semibold text-primary whitespace-pre-line">
              {p.ar}\n{p.name}
            </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
        <Card className="p-6 text-left">
          <h3 className="mb-3 text-xl font-semibold text-primary">
            🕌 Special Day Reminder - خصوصی دن کی یاد دہانی
          </h3>
          <p className="leading-relaxed">
            <strong>Day of Arafat - عرفہ کا دن</strong>
            <br />• Stand at Arafat from Dhuhr until Maghrib • ظہر سے مغرب تک
            عرفہ میں رہیں
            <br />• Make abundant dua and dhikr • خوب دعا اور ذکر کریں
            <br />• Stay hydrated and seek shade • پانی پیتے رہیں اور سائے میں
            رہیں
          </p>
        </Card>
        <Card className="p-6 text-left">
          <h3 className="mb-3 text-xl font-semibold text-primary">
            📍 Location Reminder - مقام کی یاد دہانی
          </h3>
          <p className="leading-relaxed">
            • Prayer times are for Makkah region • نمازوں کے اوقات مکہ کے علاقے
            کے لیے ہیں
            <br />• Follow your group leader's guidance • اپنے گروپ لیڈر کی
            ہدایت پر عمل کریں
            <br />• Isha combines with Maghrib in Muzdalifah • مزدلفہ میں عشاء
            مغرب کے ساتھ یکجا ہے
          </p>
        </Card>
      </div>
    </div>
...
function GuidanceSlide() {
  return (
    <div className="min-h-[calc(100vh-88px)] flex flex-col justify-center py-8">
      <SectionTitle
        children="Essential Hajj Guidance"
        subtitle="حج کی بنیادی رہنمائی"
      />
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <Card variant="accent" className="p-6">
          <h3 className="mb-4 text-center text-xl font-semibold text-primary">
            🚶 Movement Tips - حرکت اور نقل و انتقال کی ہدایات
          </h3>
          <ul className="space-y-3 text-left text-base leading-7">
            ...
            <li>
              ...
              <span className="text-primary">
                بھیڑ کی بہاؤ کی سمت پر عمل کریں
              </span>
            </li>
            <li>
        ...
        <Card variant="accent" className="p-6">
          ...
          <ul className="space-y-3 text-left text-base leading-7">
            <li>
              ...
              <br />
              <span className="text-primary">
                پانی کی بوتل (دوبارہ بھرنے کے قابل)
              </span>
            </li>
            <li>
      ...
      <Card variant="accent" className="mt-6 p-6 text-center">
        ...
        <p className="mx-auto max-w-3xl text-lg leading-relaxed">
          ...
          <br />
          <span className="text-primary">
            "اس حج کو روحانی نئی زندگی کا ذریعہ بنائیں"
          </span>
          <br />
          Remember Allah, seek forgiveness, and pray for all • الله کو یاد کریں،
          معافی مانگیں اور سب کے لیے دعا کریں
        </p>
      </Card>
    </div>
  );
...
function QiblaAzkarSlide() {
  ...
  const [deviceSupportsOrientation, setDeviceSupportsOrientation] =
    useState(true);
  ...
  useEffect(() => {
    ...
    if (!Number.isNaN(localLat) && !Number.isNaN(localLng))
      setFrom(localLat, localLng);
  ...
  const AzkarBlock = ({
    ...
    <div
      className={`overflow-hidden rounded-2xl border ${active ? "bg-primary/5" : ""}`}
    >
      <button
        onClick={onToggle}
        className="flex w-full items-center justify-between gap-3 border-b bg-muted/50 px-4 py-3 text-left"
      >
        <div className="flex items-center gap-2">
      ...
      <div
        className={`grid transition-all ${active ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}
      >
        <div className="overflow-hidden">
          <div className="space-y-3 p-4">
            {items.map((d, i) => (
              <div
                key={i}
                className="relative rounded-xl border-l-4 border-primary bg-white p-4 pr-14"
              >
                <p className="text-right text-primary leading-8" dir="rtl">
                  {d.ar}
                </p>
                <p className="text-sm text-muted-foreground">{d.en}</p>
                <span className="absolute right-3 top-3 rounded-full bg-primary/10 px-2 py-0.5 text-xs text-primary">
                  ×{d.times}
                </span>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          {deviceSupportsOrientation && !permissionAsked && (
            <div className="mb-4 text-center">
              <button
                ...
                className="rounded-xl border border-primary bg-muted/40 px-4 py-2 text-primary shadow hover:bg-primary/10 transition"
              >
                Enable Compass
              </button>
            ...
            <div className="relative h-64 w-64 rounded-full border-2 border-primary/50 bg-gradient-to-br from-primary/5 to-transparent shadow-inner sm:h-72 sm:w-72 md:h-80 md:w-80">
              ...
              {Array.from({ length: 32 }).map((_, i) => {
                ...
                return (
                  <span
                    key={i}
                    className="absolute left-1/2 top-1/2 block h-3 w-0.5 -translate-x-1/2 -translate-y-1/2 bg-primary/40"
                    style={{
                      transform: `translate(-50%, -50%) rotate(${rot}deg) translateY(-132px)`,
                    }}
                  />
                );
              })}
              <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                <div
                  className="relative h-36 w-1 origin-bottom rounded bg-gradient-to-t from-red-500 to-primary shadow transition-transform duration-500 ease-out"
                  style={{ transform: `rotate(${prevAngle}deg)` }}
                >
                  <div className="pointer-events-none absolute -top-4 left-1/2 -translate-x-1/2 text-2xl">
                    🕋
                  </div>
                </div>
              </div>
              <div className="pointer-events-none absolute inset-0">
                <span className="absolute left-1/2 top-0 -translate-x-1/2 text-xs font-semibold text-primary">
                  N • شمال
                </span>
        ...
        <div className="order-1 col-span-2 flex flex-col gap-4 lg:order-2">
          <h2 className="text-xl font-semibold text-primary">
            Morning & Evening Azkar • صبح و شام کے اذکار
          </h2>
          <AzkarBlock
          ...
          <AzkarBlock
            ...
            items={[
              {
                ar: "أَمْسَيْنَا وَأَمْسَى الْمُلْكُ لِلَّهِ رَبِّ الْعَالَمِينَ",
                en: "We have reached evening and the dominion belongs to Allah, Lord of the worlds",
                times: "1",
              },
              {
...
function DensityBar({ label, value }: { label: string; value: number }) {
  const color =
    value < 40 ? "bg-emerald-500" : value < 70 ? "bg-amber-500" : "bg-red-600";
  const text =
    value < 40
      ? "Low • منخفض"
      : value < 70
...
function StatusBadge({ status }: { status: "Open" | "Busy" | "Closed" }) {
  ...
  return (
    <span
      className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${map[status]}`}
    >
      {status}
    </span>
  );
...
function TafweejSlide() {
        ...
        arr.map((d, i) => ({
          ...
          v: Math.max(
            ...
            Math.min(
              95,
              d.v +
                (i % 2 === 0 ? 1 : -1) * (Math.floor(Math.random() * 7) - 3),
            ),
          ),
        })),
      );
  ...
  const groupPlan = [
    {
      ...
      time: "10:15",
      gate: "79",
      path: "Tunnel → Bridge L3",
      note: "Elderly priority",
    },
    {
      grp: "B",
      time: "10:45",
      gate: "80",
      path: "Tunnel → Bridge L2",
      note: "Standard flow",
    },
    {
      grp: "C",
      ...
      gate: "78",
      path: "Shari' 204 → L1",
      note: "Wheelchair lane available",
    },
  ];
  ...
  const assemblyPoint =
    minaLat && minaLng
      ? `https://maps.google.com/?q=${minaLat},${minaLng}`
      : "";
    ...
    <div className="min-h-[calc(100vh-88px)] flex flex-col justify-center py-8">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="col-span-2 space-y-4">
          <Card variant="accent" className="p-6">
            <h3 className="mb-2 text-xl font-semibold text-primary flex items-center gap-2">
              <Activity className="h-5 w-5" /> Live Crowd Density • بھیڑ کی
              کثافت
            </h3>
        ...
        <div className="space-y-4">
          <Card variant="accent" className="p-6">
            <h3 className="mb-2 text-xl font-semibold text-primary flex items-center gap-2">
              <Timer className="h-5 w-5" /> Group Departures • گروپ کے روانگی کے
              اوقات
            </h3>
          ...
          <Card variant="accent" className="p-6">
            <h3 className="mb-2 text-xl font-semibold text-primary flex items-center gap-2">
              <Route className="h-5 w-5" /> Stoning Windows • کنکریاں مارنے کے
              اوقات
            </h3>
            <ul className="space-y-2 text-sm md:text-base">
              ...
              <li className="text-red-600 flex items-center gap-2">
                <ShieldAlert className="h-4 w-4" /> Avoid peak 12:30–3:30 • پیک
                ٹائم سے بچیں 12:30–3:30
              </li>
            </ul>
          </Card>
          ...
          <Card variant="accent" className="p-6 text-center">
            <h3 className="mb-2 text-lg font-semibold text-primary">
              Assembly Point • جمع ہونے کی جگہ
            </h3>
            {assemblyPoint ? (
              <img
...
function CampInfoSlide() {
  ...
  const mapsArafat =
    arafatLat && arafatLng
      ? `https://maps.google.com/?q=${arafatLat},${arafatLng}`
      : undefined;
  ...
  return (
    <div className="min-h-[calc(100vh-88px)] flex flex-col items-center justify-center py-8">
      ...
      <SectionTitle
        children="Hajj Camps & Service Center Info"
        subtitle="حج کیمپ اور سروس کی معلومات"
      />
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <Card
          variant="accent"
          className="p-6 text-center transition-transform hover:-translate-y-0.5"
        >
          ...
          <p className="mt-2 text-lg font-bold">
            ماهر بن عبدالمقتدر بن عبدالقادر اسكندر
          </p>
        </Card>
        <Card
          variant="accent"
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        >
          ...
          <p className="text-sm font-semibold text-primary">
            Pilgrims Count • حاجیوں کی تعداد
          </p>
        ...
        >
          ...
          <p className="text-sm font-semibold text-primary">
            Pilgrims Nationality • حاجیوں کی قومیت
          </p>
          ...
          {mapsMina ? (
            <>
              <img
                className="mx-auto mt-3 h-28 w-28 rounded-lg border"
                alt="Mina QR"
                src={`https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(mapsMina)}`}
              />
            </>
          ) : (
            <p className="mt-2 text-2xl font-bold">Not set</p>
          ...
          {mapsArafat ? (
            <>
              <img
                className="mx-auto mt-3 h-28 w-28 rounded-lg border"
                alt="Arafat QR"
                src={`https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(mapsArafat)}`}
              />
            </>
          ) : (
            <p className="mt-2 text-2xl font-bold">Not set</p>
        ...
        <Card variant="accent" className="mx-auto mt-4 max-w-3xl p-6">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <label className="text-left">
              ...
              <input
                value={campNo}
                onChange={(e) => setCampNo(e.target.value)}
            ...
            <label className="text-left">
              ...
              <input
                value={minaLat}
                onChange={(e) => setMinaLat(e.target.value)}
                className="mt-1 w-full rounded-lg border px-3 py-2"
              />
            </label>
            <label className="text-left">
              ...
              <input
                value={minaLng}
                onChange={(e) => setMinaLng(e.target.value)}
                className="mt-1 w-full rounded-lg border px-3 py-2"
              />
            </label>
            <label className="text-left">
              ...
              <input
                value={arafatLat}
                onChange={(e) => setArafatLat(e.target.value)}
                className="mt-1 w-full rounded-lg border px-3 py-2"
              />
            </label>
            <label className="text-left">
              ...
              <input
                value={arafatLng}
                onChange={(e) => setArafatLng(e.target.value)}
                className="mt-1 w-full rounded-lg border px-3 py-2"
              />
            </label>
          </div>
          <div className="mt-4 flex justify-end gap-3">
            ...
            <button
              onClick={save}
              className="rounded-full border border-primary bg-primary px-4 py-2 text-white"
            >
              Save
            </button>
          </div>
    ...
    <div className="min-h-[calc(100vh-88px)] flex flex-col items-center justify-center py-8">
      ...
      <Card variant="accent" className="mt-6 p-6">
        ...
        <div className="space-y-2 text-sm md:text-base">
          ...
          <p>
            • Don’t forget to say Bismillah before eating, and Alhamdulillah
            alladhi at‘amana wa saqana after • لا تنسَ أن تقول بسم الله قبل
            الأكل، والحمد لله الذي أطعمنا وسقانا بعده
          </p>
        </div>
      </Card>
    </div>
